...
namespace Cassandra.Tests.Requests
{
    ...
    public class ResultMetadataIdTests
    {
        ...
        /// <summary>
        /// A differing id does not earn the statement its trust back. It looks as though it should - an id
        /// the server issued alongside columns is a hash of them - but which id the statement happens to
        /// hold depends on the order responses arrive in, so deriving trust from it would hand trust back to
        /// the empty-metadata id as soon as a differing one landed late. See
        /// <see cref="UpdateResultMetadata_Should_NotRestoreTrust_When_ResponsesArriveOutOfOrder"/>.
        /// </summary>
        [Test]
        public void UpdateResultMetadata_Should_NotTrustTheId_When_ColumnsArriveUnderANewOne()
        {
            var ps = ResultMetadataIdTests.PreparedWith(MetadataId, 0);
        ...
        /// </summary>
        [Test]
        public void UpdateResultMetadata_Should_TrustANewId_When_ThePrepareReportedColumns()
        {
            var ps = ResultMetadataIdTests.PreparedWith(MetadataId, 2);
            Assert.That(ExecuteRequest.ShouldSkipResultMetadata(true, ps.ResultMetadata), Is.True);
        ...
        /// <summary>
        /// The order responses arrive in must not decide whether the statement trusts its id. During a
        /// rolling upgrade one node issues a hash of empty metadata for such a statement and another a hash
        /// of the real columns, and either can land last: here the good id is published first and the
        /// empty-metadata one arrives late, carrying columns and differing from what is held. Deriving trust
        /// from the id in hand would call that one trustworthy and skip against an id the old node answers
        /// as a match for the rest of the statement's life.
        /// </summary>
        [Test]
        public void UpdateResultMetadata_Should_NotRestoreTrust_When_ResponsesArriveOutOfOrder()
        {
            var ps = ResultMetadataIdTests.PreparedWith(MetadataId, 0);

            // The node that hashed the real columns answers first.
            ps.UpdateResultMetadata(new ResultMetadata(NewMetadataId, ResultMetadataIdTests.RowSetMetadataWith(4)));

            // Then the delayed response from the node that hashed empty metadata, columns and all.
            ps.UpdateResultMetadata(new ResultMetadata(MetadataId, ResultMetadataIdTests.RowSetMetadataWith(4)));

            Assert.That(ps.ResultMetadata.ResultMetadataId, Is.EqualTo(MetadataId), "the late response won");
            Assert.That(ps.ResultMetadata.IdDescribesColumns, Is.False);
            Assert.That(
                ExecuteRequest.ShouldSkipResultMetadata(true, ps.ResultMetadata),
                Is.False,
                "an id that arrived late must not be trusted just because it differs from the one held");
        }
        ...
        /// <summary>
        /// And it is not dropped when the server later issues a different id, while the columns it comes
        /// with are still taken. One sighting of an id without columns settles the question for the
        /// statement: any later id may be the same empty-metadata hash arriving out of order, and the
        /// driver has no way to tell one from a real hash by looking at it.
        /// </summary>
        [Test]
        public void UpdateResultMetadata_Should_KeepTheMark_When_TheServerLaterIssuesADifferentId()
        {
            var ps = ResultMetadataIdTests.PreparedWith(MetadataId, 0);
            ps.UpdateResultMetadata(new ResultMetadata(MetadataId, ResultMetadataIdTests.RowSetMetadataWith(2)));
            Assert.That(ps.ResultMetadata.IdDescribesColumns, Is.False);
            ...
            Assert.That(ps.ResultMetadata.RowSetMetadata.Columns.Length, Is.EqualTo(3), "the columns are taken");
            Assert.That(ps.ResultMetadata.IdDescribesColumns, Is.False);
            Assert.That(ExecuteRequest.ShouldSkipResultMetadata(true, ps.ResultMetadata), Is.False);
        }
        ...
        /// <summary>
        /// METADATA_CHANGED on a connection that never agreed to exchange ids leaves the body with no
        /// reading that is known to be right, so the disagreement is reported rather than guessed at.
        /// Whether the field is there decides which way the guess goes wrong, and the parser cannot tell:
        /// reading one that was not written shifts everything after it forward, skipping one that was
        /// shifts it back, and either corrupts the column specs.
        /// </summary>
        /// <remarks>
        /// Both directions are covered because each is the other's mirror, and the driver read the field on
        /// the strength of the flag alone before the extension existed - so the wrong fix for one of these
        /// is exactly the previous behaviour on the other.
        /// <para>
        /// There is no legitimate cause: CQL v5 and above always exchange ids, v4 sets the flag only behind
        /// the negotiated extension, and below that bit 0x0008 is undefined. This is also where a widened
        /// or narrowed <see cref="Cassandra.Connections.IConnection.UseMetadataId"/> would first show up
        /// instead of going quiet.
        /// </para>
        /// </remarks>
        [Test]
        [TestCase(false, TestName = "the server set the flag but wrote no field")]
        [TestCase(true, TestName = "the server set the flag and wrote the field")]
        public void Rows_Should_Throw_When_MetadataChangedArrivesWithoutAnIdExchange(bool fieldWritten)
        {
            var body = ResultMetadataIdTests.RowsBody(
                RowSetMetadataFlags.MetadataChanged,
                newMetadataId: fieldWritten ? NewMetadataId : null);

            var ex = Assert.Throws<DriverInternalError>(
                () => ResultMetadataIdTests.Parse(body, useMetadataId: false));

            Assert.That(ex.Message, Does.Contain("does not exchange"));
            Assert.That(ex.Message, Does.Contain("SCYLLA_USE_METADATA_ID"));
        }

        /// <summary>
        /// And the same body on a connection that does exchange ids parses through, so the guard above is
        /// about the disagreement rather than about the flag.
        /// </summary>
        [Test]
        public void Rows_Should_ReadTheField_When_MetadataChangedArrivesWithAnIdExchange()
        {
            var body = ResultMetadataIdTests.RowsBody(
                RowSetMetadataFlags.MetadataChanged, newMetadataId: NewMetadataId);

            var response = ResultMetadataIdTests.Parse(body, useMetadataId: true);
            var metadata = ((OutputRows)response.Output).ResultRowsMetadata;

            Assert.That(response.NewResultMetadata.ResultMetadataId, Is.EqualTo(NewMetadataId));
            Assert.That(metadata.Columns.Length, Is.EqualTo(1), "nothing after the field shifted");
            Assert.That(metadata.Columns[0].Name, Is.EqualTo("col0"));
        }
        ...
        /// <summary>
        /// Zero is a count like any other. <c>columns_count</c> is mandatory and fixed-width, so a server
        /// cannot leave it unsaid - a missing one would have desynchronised the parse well before the check
        /// - and a response of no columns is no more decodable against a cache of one than a response of
        /// three would be. Treating zero as "unspecified" and decoding at the cached width would read values
        /// the response never claimed to hold.
        /// </summary>
        [Test]
        public void Rows_Should_Throw_When_NoMetadataDeclaresNoColumnsAtAll()
        {
            var body = new List<byte>();
            ...
            Assert.That(ex.Message, Does.Contain("for 0 columns"));
            Assert.That(ex.Message, Does.Contain("1 cached"));
        }
