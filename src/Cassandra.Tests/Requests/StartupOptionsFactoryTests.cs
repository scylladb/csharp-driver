...
namespace Cassandra.Tests.Requests
{
    ...
    public class StartupOptionsFactoryTests
    {
        ...
        [Test]
        public void Should_OptIntoUseMetadataId_When_TheExtensionWasNegotiated()
        {
            var factory = new StartupOptionsFactory(Guid.NewGuid(), null, null, new DriverConfigReporter(new TestConfigurationBuilder().Build()));
            var supported = Mock.Of<ISupportedOptionsInitializer>(i => i.IsMetadataIdNegotiated() == true);
        ...
        [Test]
        public void Should_NotOptIntoUseMetadataId_When_TheExtensionWasNotNegotiated()
        {
            var factory = new StartupOptionsFactory(Guid.NewGuid(), null, null, new DriverConfigReporter(new TestConfigurationBuilder().Build()));
            var supported = Mock.Of<ISupportedOptionsInitializer>(i => i.IsMetadataIdNegotiated() == false);
