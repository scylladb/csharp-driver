...
namespace Cassandra.IntegrationTests
{
    ...
    public class ScyllaMetadataIdTests : TestGlobals
    {
        ...
        /// </summary>
        /// <remarks>
        /// <c>LIST ROLES OF</c> is the shape that does this, and only with authentication enabled -
        /// without it both the statement and the CREATE ROLE before it are refused as anonymous. It
        /// reproduces on ScyllaDB 2025.3 (see scylladb/scylla-rust-driver#1575 and its fix in #1581); from
        /// 2026.1 on, the same statement declares its columns at prepare time. The assertion below holds
        /// either way, which is what makes it a regression test rather than a version probe.
        /// <para>
        /// From 2026.3 the server creates no default superuser, so this test cannot enable authentication
        /// there at all and ConnectWithRetries ignores it - measured, not inferred. Nothing is lost by
        /// that: 2026.3 declares the columns at prepare time as 2026.1 does, so the case this reproduces
        /// is not one that version exhibits.
        /// </para>
        /// </remarks>
        [Test, TestScyllaVersion(2025, 3)]
        public void Should_CacheColumns_When_ThePreparedResponseCarriesNoResultMetadata()
        {
            ...
            // Deliberately no -Dcassandra.superuser_setup_delay_ms=0, which is what
            // SessionAuthenticationTests passes: that is a Cassandra JVM flag, and Scylla's ccm refuses to
            // start a node given it. SessionAuthenticationTests never meets that, being excluded from the
            // Scylla leg. Whether the superuser exists is therefore left to the server, and handled by
            // ConnectWithRetries.
            realCluster.Start();
            try
            {
                using (var cluster = ConnectWithRetries(realCluster))
                {
                    ...
                    // Which shape the server owes, decided from its version rather than read off the
                    // statement under test. Reading it off the statement would let the test switch
                    // branches and still pass if the driver ever wrongly reported columns at prepare -
                    // the selector would come from the very thing being checked. Asserting it instead
                    // means a server that stops behaving as its version says fails here, saying so.
                    var expectEmptyPrepare = TestClusterManager.ScyllaVersion < new Version(2026, 1);
                    Assert.AreEqual(
                        expectEmptyPrepare,
                        !ps.ResultMetadata.ContainsColumnDefinitions(),
                        expectEmptyPrepare
                            ? "this version reports no result metadata for LIST ROLES OF, so the recovery " +
                              "is what this test exercises - it has stopped doing so"
                            : "this version declares the columns at prepare time, so there is no empty " +
                              "metadata to recover from - it has started reporting none");
        ...
        /// <summary>
        /// Connects with the default superuser, waiting for it in case the server creates it after it starts
        /// accepting connections.
        /// </summary>
        /// <remarks>
        /// Measured on 2025.3 and 2026.1, the first attempt succeeds, so the budget is small: it covers a
        /// server that creates the superuser slightly late, not one that never does. 2026.3 never does -
        /// it rejects the credentials outright rather than timing out - and a long wait there only delays
        /// the inevitable skip by a minute on every run of the newest CI leg.
        /// <para>
        /// Running out of attempts ignores the test rather than failing it. A default superuser is an
        /// assumption about the server, not something this driver promises, and 2026.3 offers no way to
        /// supply one over CQL - scylla.yaml has no initial-superuser option and the documented route is
        /// the maintenance socket, which this driver does not speak. A release that stops creating one
        /// would otherwise turn that leg red with a message pointing nowhere near the cause.
        /// </para>
        /// <para>
        /// What a skip costs is the reproduction of the empty-prepared-metadata case, which needs
        /// LIST ROLES OF and so needs authentication. The rule it checks is covered without
        /// authentication too, by <see cref="Should_NotAcquireResultMetadata_When_TheStatementReturnsNoRows"/>
        /// and by the IdDescribesColumns tests in <c>ResultMetadataIdTests</c>.
        /// </para>
        /// </remarks>
        private ICluster ConnectWithRetries(ITestCluster realCluster)
        {
            ...
            Assert.Ignore(
                "Could not authenticate as the default 'cassandra' superuser within the wait, so this " +
                "server does not create one and the test cannot enable authentication. Last error: " +
                last);
            return null;
        }
