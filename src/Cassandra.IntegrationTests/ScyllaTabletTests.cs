...
namespace Cassandra.IntegrationTests
{
    ...
    public class ScyllaTabletTest : TestGlobals
    {
        ...
        public void CorrectTabletMapTest()
        {
            ...
            _session.Execute("CREATE KEYSPACE tablettest WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': '1'}  AND tablets = {'initial': 4};");
            ...
            if (tabletMap.GetMapping().TryGetValue(key, out var tabletSet))
            {
                ...
                Assert.IsTrue(tablets.Count == 4, "There should be 8 tablets for the table as stated in the keyspace definition");
            }
        }
    }
}
